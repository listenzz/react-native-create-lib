...
module.exports = {
  ...
  func: (args, config, options) => {
    ...
    const repoName = createOptions.repoName
...
${emoji.get('clock9')}  It took ${Date.now() - beforeCreation}ms.`)
  ...
  options: [
    ...
    {
      command: '--repo-name [repoName]',
      description: 'The repository name. (default: "react-native-(name in param-case)")',
    },
    {
      command: '--example-name [exampleName]',
      description: 'The example name. (default: "(name in pascal-case)Example" )',
    },
    {
