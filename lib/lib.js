...
const generateWithNormalizedOptions = (
  ...
  name: ${name}
  className: ${className}
  clsssNameWithPrefix: ${clsssNameWithPrefix}
  ...
  const generateExampleApp = () => {
    return Promise.resolve()
      .then(() => {
        ...
        return Promise.all([
          fs.copy(path.join(exampleDir, 'android'), path.join(moduleName, 'example/android')),
          fs.copy(path.join(exampleDir, 'ios'), path.join(moduleName, `example/ios/${className}`)),
        ]);
      })
      .then(() => {
        ...
        return Promise.all(
          exampleTemplates.map(template => {
            return renderTemplateIfValid(fs, path.join(moduleName, 'example'), template, templateArgs);
          }),
        );
      })
      .then(() => {
      ...
      .then(() => {
        const execOptions = { cwd: `./${moduleName}/example/ios`, stdio: 'inherit' };
